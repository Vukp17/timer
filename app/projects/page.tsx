...
import type { Column } from "@/models/data-table"
...
import type { Project, ProjectCreate, ProjectStatus } from "../models/project"
...
export default function ProjectManagement() {
  ...
  useEffect(() => {
    getProjectList(page, searchTerm, sortField, sortOrder)
      .then((data) => {
        if (data !== undefined) {
    ...
    getAll()
      .then((data) => {
        if (data !== undefined) {
          setClients(data)
          projectFields.find((field) => field.name === "clientId")!.options = data.map((client) => ({
            id: client.id,
            label: client.name,
          }))
        } else {
  ...
  const handleSort = (column: string, direction: string) => {
    ...
    getProjectList(page, searchTerm, column, direction)
      .then((data) => {
        if (data !== undefined) {
          setProjects(data)
        } else {
          ...
          currentProject
            ? {
                ...
                clientId: currentProject.clientId
                  ? clients.find((client) => client.id === currentProject.clientId)?.id.toString() || ""
                  : "",
              }
            : undefined
        }
